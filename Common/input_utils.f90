...
!    call write_occupations(kp)
  ...
  !---------------------------------------------------------------------------------------------------
  !      Computes band occupations for all bands and outputs these quantities in the array occ.
  !      Will find occupations based on MP-1 smearing scheme if provided the
  !      flag do_smearing = .true.
  !      if do_smearing = .false. (default) then computes occupations as
  !      either 0, 0.5, or 1 based on position relative to fermi energy. 
  !      This flag is set to true when occ_broadening is specified in the input file.  
  subroutine find_occupations(el, nspin, nrk, nband, efermi, occ_broadening, occ, do_smearing)
    real(DP), intent(in) :: el(:,:,:) ! kp%el or kpq%el
    integer, intent(in) :: nspin
    integer, intent(in) :: nrk
    integer, intent(in) :: nband
    real(DP), intent(in) :: efermi
    real(DP), intent(in) :: occ_broadening
    real(DP), allocatable, intent(inout) :: occ(:,:,:,:)
    logical, intent(in) :: do_smearing

    real(DP) :: en_fac, e_band, occ_ib
    integer :: ib, irk, ispin

    PUSH_SUB(find_occupations)

    ! OAH: occ has extra index hard-coded to 1 for frequency,
    !      which will be a later development.
    SAFE_ALLOCATE(occ, (nband, nrk, nspin, 1))
   
    ! OAH: partial occupations block (smearing)
    if (do_smearing) then
      do ispin = 1, nspin 
        do irk = 1, nrk
          do ib = 1, nband
            e_band = el(ib, irk, ispin)
            en_fac = (e_band * ryd - efermi) / occ_broadening
            occ_ib = 0.5*(1-erf(en_fac))-en_fac/(2*sqrt(PI_D))*exp(-en_fac**2)
            occ(ib, irk, ispin, 1) = occ_ib
          enddo
        enddo 
      enddo 
    ! OAH: no smearing occupations block:
    else
      do ispin = 1, nspin
        do irk = 1, nrk
          do ib = 1, nband 
            e_band = el(ib, irk, ispin) 
            if (e_band*ryd > efermi + TOL_Degeneracy) then
              occ_ib = 0d0
            else if (e_band*ryd < efermi - TOL_Degeneracy) then
              occ_ib = 1d0
            else
              occ_ib = 0.5  ! within TOL_Degeneracy of the Fermi level, use FD(E_F) = 1/2
            endif
            occ(ib, irk, ispin, 1) = occ_ib
          enddo
        enddo
    ...
    POP_SUB(find_occupations)

  end subroutine ! find_occupations

  !---------------------------------------------------------------------------------------------------
  !     Finds nband and ncrit quantities for partial occupation calculations.  
  subroutine get_nband_ncrit(nspin, nrk, totnband, nband, ncrit, occs, kpq, occqs)
    integer, intent(in) :: nspin
    integer, intent(in) :: nrk
    integer, intent(in) :: totnband ! kp%mnband for epsilon or sig%ntband for sigma
    integer, intent(inout) :: nband ! number of fully occupied bands
    integer, intent(inout) :: ncrit
    real(DP), intent(in) :: occs(:,:,:,:) 
    type(kpoints), intent(in), optional :: kpq
    real(DP), intent(in), optional :: occqs(:,:,:,:)

    integer :: ncrit_band, ncrit_band_temp, nband_temp
    integer :: ispin, ik, ib
    real(DP) :: occ

    PUSH_SUB(get_nband_ncrit)

    ncrit_band = 0

    ! OAH The below block works as follows:
    ! nband: the number of bands fully occupied across all kpoints, so can only go down as we iterate through kpoints
    ! ncrit_band: index of the highest partially occupied band, so can only go up as we iterate through kpoints
    !   so ncrit = ncrit_band (index of highest partially occupied band) -  nband (number of fully occupied bands)
    ! Note that negative occupations are counted as partial occupations, and
    ! occupations > 1 are counted as full occupations.
    do ispin = 1, nspin
      do ik = 1, nrk
        ncrit_band_temp = 0
        nband_temp = 0
        do ib = 1, totnband
          occ = occs(ib, ik, ispin, 1)
          if(abs(occ) > TOL_Zero .and. (1d0-occ) .ge. TOL_Small) then
            ncrit_band_temp = ib
          endif
          if((1d0-occ) < TOL_Small .or. (occ-1d0) > TOL_Zero) then
            nband_temp = nband_temp + 1
          endif
        enddo
        if(ncrit_band_temp > ncrit_band) ncrit_band = ncrit_band_temp
        if(ik .eq. 1 .and. ispin .eq. 1) nband = nband_temp
        if(nband_temp < nband) nband = nband_temp
      enddo
    enddo 
    if (present(kpq)) then
      do ispin = 1, kpq%nspin
        do ik = 1, kpq%nrk
          ncrit_band_temp = 0
          nband_temp = 0
          do ib = 1, kpq%mnband
            occ = occqs(ib, ik, ispin, 1)
            if(occ > TOL_Small .and. (1d0-occ) > TOL_Small) then
              ncrit_band_temp = ib
            endif
            if((1d0-occ) < TOL_Small .or. (occ-1d0) > TOL_Zero) then
              nband_temp = nband_temp + 1
    ...
    if (ncrit_band .eq. 0) then
      ncrit_band = nband
    endif
    ncrit = ncrit_band - nband
  
    POP_SUB(get_nband_ncrit)

  end subroutine ! get_nband_ncrit
