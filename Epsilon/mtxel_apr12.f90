...
subroutine mtxel_cpu(iv,gvec,vwfn,cwfn,pol,ispin,irk,kp,kpq,rank_mtxel,kfact)
  ...
  ! WK: for energy window
  real(DP) :: eval, econd  
  real(DP), intent(in) :: ewin_min, ewin_max
  ! WK: for energy window
...
!$OMP END PARALLEL DO
      ...
      call update_gme(tmparray, kp, eden, kfact, iv, ic, jsp, ic_loc, &
                      iv_loc, ispin, irk, freq_idx, &       !WK
                      eval, econd, ewin_min, ewin_max, pol) !WK
...
subroutine update_gme(tmparray, kp, eden, kfact, iv, ic, jsp, ic_loc, &
                      iv_loc, ispin, irk, freq_idx, &
                      eval, econd, ewin_min, ewin_max, pol)
  implicit none
